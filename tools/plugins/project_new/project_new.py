...
#
class KKPluginNew(kkThree.KKPlugin):

    DEFAULT_PROJ_NAME = {
        kk_project.Project.JS: 'MyJSGame'
    }
    ...
    def init(self, args):
        ...
        self._kkroot = self.get_cocos2d_path()

        # search for custom paths
        if args.engine_path is not None:
            self._kkroot = os.path.abspath(args.engine_path)
            self._kkroot = unicode(self._kkroot, "utf-8")
            tp_path = os.path.join(self._kkroot, "templates")
        ...
        self._templates = Templates("js", self._templates_paths, args.template)
    ...
    def parse_args(self, argv):
        ...
        name = KKPluginNew.plugin_name()
        category = KKPluginNew.plugin_category()
        parser = ArgumentParser(prog="kk %s" % self.__class__.plugin_name(),
        ...
        if args.name is None:
            args.name = KKPluginNew.DEFAULT_PROJ_NAME[args.language]
    ...
    def _stat_engine_version(self):
        try:
            ...
            framework_ver_file = os.path.join(self._kkroot, 'version')
            x_ver_file = os.path.join(self._kkroot, 'cocos/cocos2d.cpp')
            js_ver_file = os.path.join(self._kkroot, 'frameworks/js-bindings/bindings/manual/ScriptingCore.h')
    ...
    def _create_from_cmd(self):
        ...
        if os.path.exists(self._projdir):
            ...
            raise kkThree.KKPluginError(message)

        tp_dir = self._templates.template_path()

        creator = TPCreator(self._lang, self._kkroot, self._projname, self._projdir,
        ...
        data = None
        cfg_path = os.path.join(self._projdir, kk_project.Project.CONFIG)
        ...
        if kk_project.Project.KEY_PROJ_TYPE not in data:
            data[kk_project.Project.KEY_PROJ_TYPE] = self._lang

        # script project may add native support
        data[kk_project.Project.KEY_HAS_NATIVE] = False
...
class Templates(object):
    ...
    def select_one(self):
        kkThree.Logging.warning('Multiple templates detected!')
        kkThree.Logging.warning(
            "You can select one via command line arguments (-h to see the options)")
        kkThree.Logging.warning('Or choose one now:\n')

        p = self._template_folders.keys()
        for i in range(len(p)):
            kkThree.Logging.warning('%d %s' % (i + 1, p[i]))
        kkThree.Logging.warning("Select one (input number and press enter): ")
        while True:
            option = raw_input()
            if option.isdigit():
                option = int(option) - 1
...
class TPCreator(object):
    ...
    def cp_self(self, project_dir, exclude_files):
        kkThree.Logging.info('> Copy template into %s' % project_dir)
        ...
        kkThree.copy_files_with_config(copy_cfg, self.tp_dir, self.project_dir)
    ...
    def do_other_step(self, step, not_existed_error=True):
        if step not in self.tp_other_step:
            if not_existed_error:
                ...
                # handle as warning
                kkThree.Logging.warning("WARNING: Can't find step %s." % step)
                return
    ...
    def do_cmds(self, cmds):
        for k, v in cmds.iteritems():
            ...
            try:
                ...
                raise kkThree.KKPluginError("cmd = %s is not found" % k)

            try:
                cmd(v)
            except Exception as e:
                raise kkThree.KKPluginError(str(e))
...
# cmd methods below
    def append_h5_engine(self, v):
        ...
        # begin copy engine
        kkThree.Logging.info("> Copying kkThree framework files...")
        for index in range(len(file_list)):
            srcfile = os.path.join(src, file_list[index])
            dstfile = os.path.join(dst, file_list[index])

            srcfile = kkThree.add_path_prefix(srcfile)
            dstfile = kkThree.add_path_prefix(dstfile)

            if not os.path.exists(os.path.dirname(dstfile)):
                os.makedirs(kkThree.add_path_prefix(os.path.dirname(dstfile)))
    ...
    def append_x_engine(self, v):
        ...
        if kkThree.KKPlugin.get_cocos2d_mode() == 'distro':
            return
        ...
        # begin copy engine
        kkThree.Logging.info("> Copying cocos2d-x files...")
    ...
    def append_from_template(self, v):
        kkThree.Logging.info('> Copying files from template directory...')
        kkThree.copy_files_with_config(v, self.tp_dir, self.project_dir)

    def append_dir(self, v):
        kkThree.Logging.info('> Copying directory from cocos root directory...')
        for item in v:
            kkThree.copy_files_with_config(
                item, self.cocos_root, self.project_dir)

    def append_file(self, v):
        kkThree.Logging.info('> Copying files from cocos root directory...')
        for item in v:
            src = os.path.join(self.cocos_root, item['from'])
            dst = os.path.join(self.project_dir, item['to'])

            src = kkThree.add_path_prefix(src)
            dst = kkThree.add_path_prefix(dst)
...
# project cmd
    def project_rename(self, v):
        ...
        kkThree.Logging.info("> Rename project name from '%s' to '%s'" % (
    ...
    def project_replace_project_name(self, v):
        ...
        dst_project_name = self.project_name
        src_project_name = v['src_project_name']
        kkThree.Logging.info("> Replace the project name from '%s' to '%s'" % (
            src_project_name, dst_project_name))
        files = v['files']
        for f in files:
            dst = f.replace("PROJECT_NAME", dst_project_name)
    ...
    def project_replace_package_name(self, v):
        ...
        kkThree.Logging.info("> Replace the project package name from '%s' to '%s'" % (
            src_package_name, dst_package_name))
        files = v['files']
        if not dst_package_name:
            raise kkThree.KKPluginError('package name not specified')
        for f in files:
            dst = f.replace("PROJECT_NAME", dst_project_name)
            if os.path.exists(os.path.join(dst_project_dir, dst)):
                replace_string(
    ...
    def modify_files(self, v):
        ...
        """
        kkThree.Logging.info("> Modify files by re.sub()")
        for modify_info in v:
            ...
            if not os.path.isfile(modify_file):
                kkThree.Logging.warning("%s is not a file." % modify_file)
                continue
