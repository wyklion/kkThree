...
'''
Command line tool manager for kkThree
'''
...
class Cocos2dIniParser:
    def __init__(self):
        ...
        self.kk_path = os.path.dirname(os.path.abspath(sys.argv[0]))
        self._cp.read(os.path.join(self.kk_path, "kkThree.ini"))

        # XXXX: override with local config ??? why ???
        self._cp.read("~/.kkThree/cocos2d.ini")
    ...
    def _sanitize_path(self, path):
        ...
        path = os.path.abspath(os.path.join(self.kk_path, path))
        if not os.path.isdir(path):
            Logging.warning("Warning: Invalid directory defined in kkThree.ini: %s" % path)
            return None
        return path
...
#
# Plugins should be a sublass of KKPlugin
#
class KKPlugin(object):
    ...
    def init(self, args):
        ...
        self._platforms = kk_project.Platforms(self._project, args.platform)
    ...
    def parse_args(self, argv):
        ...
        parser.add_argument("-q", "--quiet",
                            ...
                            dest="quiet",
                            help="less output")
        platform_list = kk_project.Platforms.list_for_display()
        ...
        if args.src_dir is None:
            self._project = kk_project.Project(os.path.abspath(os.getcwd()))
        else:
            self._project = kk_project.Project(
