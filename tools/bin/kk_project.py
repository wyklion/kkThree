...
class Project(object):
    ...
    def _parse_project_json(self, src_dir):
        ...
        if proj_path == None:
            raise kkThree.KKPluginError("Can't find config file %s in path %s" % (Project.CONFIG, src_dir))
        ...
        try:
            ...
            if f is not None:
                f.close()
            raise kkThree.KKPluginError("Configuration file %s is broken!" % project_json)

        if project_info is None:
            raise kkThree.KKPluginError("Parse configuration in file \"%s\" failed." % Project.CONFIG)

        if not project_info.has_key(Project.KEY_PROJ_TYPE):
            raise kkThree.KKPluginError("Can't get value of \"%s\" in file \"%s\"." % (Project.KEY_PROJ_TYPE, Project.CONFIG))
        ...
        if not (lang in Project.language_list()):
            raise kkThree.KKPluginError("The value of \"%s\" must be one of (%s)" % (Project.KEY_PROJ_TYPE, ', '.join(Project.list_for_display())))
        ...
        if (project_info.has_key(Project.KEY_CUSTOM_STEP_SCRIPT)):
            ...
            if os.path.isfile(script_path):
                ...
                kkThree.Logging.info("Find custom step script: %s" % script_path)
            else:
                kkThree.Logging.warning("Can't find custom step script %s" % script_path)
                self._custom_step = None
    ...
    def invoke_custom_step_script(self, event, tp, args):
        try:
            ...
            kkThree.Logging.warning("Custom step invoke failed: %s" % e)
            raise e

    def _find_project_dir(self, start_path):
        path = start_path
        while True:
            if kkThree.os_is_win32():
...
class Platforms(object):
    ...
    CFG_CLASS_MAP = {
        ANDROID : "kk_project.AndroidConfig",
        IOS : "kk_project.iOSConfig",
        MAC : "kk_project.MacConfig",
        WEB : "kk_project.WebConfig",
        WIN32 : "kk_project.Win32Config",
        LINUX : "kk_project.LinuxConfig",
        WP8 : "kk_project.Wp8Config",
        WP8_1 : "kk_project.Wp8_1Config",
        METRO : "kk_project.MetroConfig"
    }
    ...
    def __init__(self, project, current):
        ...
        if current is not None:
            ...
            if current_lower in self._available_platforms.keys():
                ...
                raise kkThree.KKPluginError("Current available platforms : %s. '%s' is not available." % (self._available_platforms.keys(), current))
    ...
    def _filter_platforms(self, platforms):
        ...
        for p in platforms:
            if kkThree.os_is_linux():
                if p in platforms_for_os["linux"]:
                    ret.append(p)
            if kkThree.os_is_mac():
    ...
    def _gen_available_platforms(self, proj_info):
        ...
        for p in platform_list:
            cfg_class = kkThree.get_class(Platforms.CFG_CLASS_MAP[p])
        ...
        if len(self._available_platforms) == 0:
            raise kkThree.KKPluginError("There isn't any available platforms")
    ...
    def select_one(self):
        ...
        raise kkThree.KKPluginError("The target platform is not specified.\n" +
